...
function Updateform() {
    ...
    const [updateMobile, setUpdateMobile] = useState(true);
    ...
    const [mobileDoesntExists, setMobileDoesntExists] = useState(true)
    useEffect(() => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.log('verified  number is',location.state.userPhoneNumber)
            if (updateMobile) {
                console.log('entered')
                formData.newNumber = location.state.userPhoneNumber
                formData.oldNumber = oldNumber
                formData.updateMobileNumber =true
            }
            else {
                formData.updateMobileNumber =false
                formData.newNumber = oldNumber
                formData.oldNumber = oldNumber
            }
                    ...
                    <form onSubmit={handleSubmit} className='w-full  text-black text-center  flex flex-col p-2'>
                        ...
                        <div className="form-control">
                            <label className="label cursor-pointer">
                                <span className="label-text">Update mobile to new verified number</span>
                                <input type="checkbox" defaultChecked onChange={() => setUpdateMobile(!updateMobile)} className="checkbox" />
                            </label>
                        </div>
